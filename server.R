...
sampleTypes <- c("Drain Water"=1, "Produce"=2, "Piped Water"=3,
                 ...
                 "Public Latrine Surfaces"=7, "Particulate"=8, "Bathing"=9, "Street Food"=10)
...
#   # Download the data ----------------------------------------------------------------
  ...
  ec_data <- eventReactive(lab_data(), {
    withProgress(
      create_ecData(collection_data = collection_data(), lab_data = lab_data()),
      message = 'Calculating Concentration Data', value = 50)
  })

  conc <- eventReactive(ec_data(), { #This called create_ecData again. Maybe we should simplify it.
    withProgress(
      create_concData(collection_data = collection_data(), lab_data = lab_data(), pathway_selected_vector = pathway_selected_vector()), # place holder for vector of selected pathways.
